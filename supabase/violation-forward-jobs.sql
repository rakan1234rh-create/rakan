-- ═══════════════════════════════════════════════════════════════════════════
-- ATHAR — تمرير تلقائي عند الموعد فقط (جدول jobs عند تغيير المرحلة)
-- شغّل بعد mirsad-auto-forward-sql.sql
-- يستبدل المسح كل دقيقة لكل المخالفات بمعالجة jobs المستحقة فقط.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ─── إلغاء jobs معلّقة لاتجاه معيّن ───
...
-- ─── استبدال tick: jobs فقط (بدل مسح كل المخالفات) ───
...
declare
  v_res jsonb;
begin
  v_res := public.mirsad_process_forward_jobs(50);
  return v_res;
...
-- اختبار:
-- select public.mirsad_process_forward_jobs(10);
