-- Restroom schedules define the duration of each visit. A completed restroom
-- visit must not consume or block a later restroom visit on the same day.
...
BEGIN
  ...
  -- A restroom schedule is a per-visit timer. Always start a fresh visit with
  -- the currently resolved duration, regardless of earlier restroom visits.
  IF v_type = 'restroom' THEN
    ...
    IF v_mins IS NULL THEN
      RETURN jsonb_build_object(
        ...
        'error', 'لا يوجد بريك دورة مياه مجدول لهذا اليوم'
      );
    END IF;
    ...
    RETURN jsonb_build_object(
      'ok', true, 'resumed', false, 'new_session', true, 'break', to_jsonb(v_row)
    );
  END IF;

  -- Regular breaks keep their existing once-per-day remaining balance.
  SELECT * INTO v_prev
  FROM public.staff_breaks
  WHERE user_id = v_uid
  ...
  IF EXISTS (
    ...
    RETURN jsonb_build_object(
      'ok', false, 'exhausted', true, 'error', 'خلصت مدة بريك اليوم'
    );
  END IF;
...
COMMENT ON FUNCTION public.start_staff_break(text) IS
  'Starts a daily-balance regular break or a fresh per-visit restroom break.';
